...
Meteor.methods({
  ...
  eventConfirmToggle: function(eventId, personId) {
    ...
    if (eventId.length === 17 && personId.length === 17) {
      ...
      // If the current person has been confirmed for this event before, update the confirm, this also means the event must have a "confirms" array so no more trouble there.
      else {
        Events.update({
          _id: eventId,
